class Matrix_2x2:
	...
	def add(self, matrix2: 'Matrix_2x2') -> 'Matrix_2x2':
		if (isinstance(matrix2, Matrix_2x2)):
		    res = [[0, 0], [0, 0]]
		    for i in range(2):
		        for j in range(2):
		            res[i][j] = self.matrix[i][j] + matrix2.matrix[i][j]
		    return Matrix_2x2(res[0][0], res[0][1],	res[1][0], res[1][1])
		else : 
			raise TypeError('{matrix2} is not a Matrix_2x2')

	def prod_r(self, r: float | int) -> 'Matrix_2x2':
		for i in range(2):  
        	for j in range(2):
            	self.matrix[i][j] *= r
        return Matrix_2x2(res[0][0], res[0][1], res[1][0], res[1][1])

	def prod(self, matrix2: 'Matrix_2x2') -> 'Matrix_2x2':
        if isinstance(matrix2, Matrix_2x2):
            a = self.matrix
            b = matrix2.matrix
